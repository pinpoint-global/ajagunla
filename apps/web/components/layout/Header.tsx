...
export const Header = ({ className, ...props }: HeaderProps) => {
  ...
  const [desktopLogoErrored, setDesktopLogoErrored] = useState(false);
  const pathname = usePathname();
  const branding = useSiteBranding();
  const { navLinks } = useSiteGlobalContent();
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center">
                  ...
                  <div className="relative hidden lg:flex items-center">
                    {!branding.useLocalDesktopLogo &&
                    branding.desktopLogoUrl &&
                    !desktopLogoErrored ? (
                      <img
                        src={branding.desktopLogoUrl}
                        alt={branding.logoAlt}
                        width={155}
                        height={40}
                        className="h-10 w-auto object-contain"
                        onError={() => setDesktopLogoErrored(true)}
                      />
                    ) : (
          ...
          <div className="hidden lg:flex items-center space-x-8">
            <nav className="">
              <ul className="list-none hidden lg:flex items-center space-x-8">
                {navLinks
                  .filter(s => !s.showInFooterOnly)
                  .map((item, idx) => (
                    <HeaderLink key={idx} {...item} activePath={pathname ?? undefined} />
                  ))}
              </ul>
            </nav>
