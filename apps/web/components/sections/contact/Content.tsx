...
export const ContactPageContent = () => {
  ...
  const { contactCards, contactInformation, socials } = useSiteGlobalContent();
...
export interface ContactPageContentProps {
  contactCardsOverride?: ContactCardProps[];
}

export const ContactPageContentWithOverrides = ({
  contactCardsOverride,
}: ContactPageContentProps) => {
  const { siteLoading } = useSiteStore(state => state);
  const { contactCards, contactInformation, socials } = useSiteGlobalContent();
  const cards = contactCardsOverride ?? contactCards;
...
const ContactInfoCard = ({ iconName, title, href, texts, allowSameRow }: ContactCardProps) => {
  return (
    <div className="flex items-start gap-3">
      <DynamicIcon
        name={iconName}
        props={{ className: 'w-5 h-5 text-primary mt-1 flex-shrink-0' }}
      />
