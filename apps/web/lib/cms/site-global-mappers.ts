...
export function mapSiteBranding(doc: SiteGlobalDocInput | null): SiteBranding {
...
function asContactInformation(value: unknown): SiteContactInformation | null {
  ...
  const address = asStringArray(v.address);
  const constituencyOffice = asStringArray(v.constituencyOffice);
  const tel = asStringArray(v.tel);
  const email = asStringArray(v.email);
...
export function mapSiteGlobalContent(doc: SiteGlobalDocInput | null): SiteGlobalContent {
